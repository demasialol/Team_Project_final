...
## 2-2 리얼타임 데이터베이스
+ 소개<br>
Realtime Database는 Firebase에서 제공하는 실시간 데이터베이스로 데이터 저장 및 동기화가 실시간으로 가능한 데이터베이스입니다. 이 앱에서는 회원가입한 사용자의 일부 정보를 저장하고, 전송된 이미지를 스트링 값으로 변환하여 저장하는 곳에 사용했습니다.<br>
+ 데이터 저장<br>
1. 회원가입한 사용자의 정보 중 이름과 국가 정보 저장
2. 이미지 전송시, 이미지를 스트링 값으로 변환하여 저장
+ 데이터 불러오기<br>
